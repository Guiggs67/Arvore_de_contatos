...
private:
    ...
    Node* remove(Node* node, std::string nome) {
        ...
        std::cout << "Contato removido com sucesso" << endl;
        if (node == nullptr)
            return node;
...
public:
    ...
    void ExportarCSV() {
        ...
        outFile.close();
        cout << "Contatos exportados com sucesso para 'contatos.csv'." << endl;
    }

    void ImportarCSV() {
        ifstream inFile("contatos.csv");
        if (!inFile) {
        ...
        inFile.close();
        cout << "Contatos importados com sucesso de 'contatos.csv'." << endl;
    }
